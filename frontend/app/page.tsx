      ...
      <section className="workspace">
        ...
        <section className="panel soak-panel" id="soak">
          ...
          {!soakStatus?.campaign || !soakStatus.metrics ? <div className="soak-empty"><p>Nenhuma campanha foi iniciada. O teste acompanha dados, sinais e limites por 7 dias sem habilitar compras automáticas.</p><button onClick={() => void startSoakCampaign()} disabled={busy}>Iniciar teste seguro</button></div> : <>
            ...
            <div className={`soak-monitor-state ${(soakStatus.metrics.monitoring?.active_alerts?.length ?? 0) > 0 ? "alert" : "healthy"}`}><strong>{(soakStatus.metrics.monitoring?.active_alerts?.length ?? 0) > 0 ? `${soakStatus.metrics.monitoring!.active_alerts!.length} alerta(s) ativo(s)` : "Monitor de segurança normal"}</strong><span>Última verificação: {soakStatus.metrics.monitoring?.last_checked_at ? new Date(soakStatus.metrics.monitoring.last_checked_at).toLocaleString("pt-BR") : "aguardando o próximo ciclo"}</span></div>
